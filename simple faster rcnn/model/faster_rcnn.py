...
class FasterRCNN(nn.Module):
    ...
    def _suppress(self, raw_cls_bbox, raw_prob):
        ...
        for l in range(1, self.n_class):
            cls_bbox_l = raw_cls_bbox.reshape((-1, self.n_class, 4))[:, l, :]  # <class 'tuple'>: (300, 4)
            prob_l = raw_prob[:, l]  # <class 'tuple'>: (300,)
            mask = prob_l > self.score_thresh
            cls_bbox_l = cls_bbox_l[mask]
            prob_l = prob_l[mask]
            # modified!
            keep = nms(t.from_numpy(cls_bbox_l[:, [1, 0, 3, 2]]), t.from_numpy(prob_l), self.nms_thresh)
            # keep = non_maximum_suppression(cp.array(cls_bbox_l), self.nms_thresh, prob_l)

            bbox.append(cls_bbox_l[cp.asnumpy(keep)])
    ...
    def predict(self, imgs, sizes=None, visualize=False):
        ...
        for img, size in zip(prepared_imgs, sizes):
            ...
            mean = t.Tensor(self.loc_normalize_mean).to(roi_cls_loc.device).repeat(self.n_class)[None]
            std = t.Tensor(self.loc_normalize_std).to(roi_cls_loc.device).repeat(self.n_class)[None]
